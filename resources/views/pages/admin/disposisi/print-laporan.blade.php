        ...
        <div class="col">
          ...
          <table class="table table-striped table-bordered table-sm" width="100%">
            <thead>
              ...
              <th style="text-align: center;font-size: small;">Surat Dari</th>
              <th style="text-align: center;font-size: small;">Tanggal Surat</th>
              <th style="text-align: center;font-size: small;">No. Agenda</th>
              <th style="text-align: center;font-size: small;">Diterima Tanggal</th>
            ...
            <tbody>
              @foreach ($item as $letter)
                <tr>
                  <td style="text-align: center;font-size: small;">{{ $loop->iteration }}</td>
                  <td style="text-align: center;font-size: small;">{{ $letter->letter->letter_no }}</td>
                  <td style="text-align: center;font-size: small;">{{ $letter->agency->name }}</td>
                  <td style="text-align: center;font-size: small;">{{ Carbon\Carbon::parse($letter->tgl_surat)->translatedFormat('l, d F Y') }}</td>
                  <td style="text-align: center;font-size: small;">{{ $letter->no_agenda }}</td>
                  <td style="text-align: center;font-size: small;">
                    {{ Carbon\Carbon::parse($letter->letter->date_received)->translatedFormat('l, d F Y') }}</td>
                  <td style="text-align: center;font-size: small;">
                    {{ Carbon\Carbon::parse($letter->letter->tgl_selesai)->translatedFormat('l, d F Y') }}</td>
                  <td style="text-align: center;font-size: small;">
                    {{ Carbon\Carbon::parse($letter->letter->tgl_aju_kembali)->translatedFormat('l, d F Y') }}</td>
                </tr>
              @endforeach
            </tbody>
          </table>
